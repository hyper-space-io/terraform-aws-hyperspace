resource "null_resource" "wait_for_nlb" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      until STATE=$(aws elbv2 describe-load-balancers --load-balancer-arns ${data.aws_lb.argocd_lb.arn} --query 'LoadBalancers[0].State.Code' --output text) && [ "$STATE" = "active" ]; do
  ...
  triggers = {
    nlb_arn = data.aws_lb.argocd_lb.arn
  }
}
...
resource "aws_vpc_endpoint_service" "argocd_server" {
  ...
  network_load_balancer_arns = [data.aws_lb.argocd_lb.arn]
