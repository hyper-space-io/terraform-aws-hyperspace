resource "null_resource" "wait_for_nlb" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      until STATE=$(aws elbv2 describe-load-balancers --load-balancer-arns ${data.argocd_lb.nlb.arn} --query 'LoadBalancers[0].State.Code' --output text) && [ "$STATE" = "active" ]; do
...
resource "aws_vpc_endpoint_service" "argocd_server" {
  ...
  network_load_balancer_arns = [data.argocd_lb.nlb.arn]
